...
# - selects ONE quote PER channel
# - posts one combined Discord message with all selected quotes
...
# - Outputs:
#   1) quote_run.json (debug, includes per-channel results)
#   2) quotes_selected_only.json (APPEND-ONLY archive, only meta + selected_per_channel)
# =============================================================================
...
# optional
DISCORD_MAX_MESSAGE_LEN = int(os.getenv("QUOTE_DISCORD_MAX_MESSAGE_LEN", "1900"))
QUOTE_MAX_TEXT_LEN = int(os.getenv("QUOTE_MAX_TEXT_LEN", "300"))
...
# Discord post format
# =============================================================================
def format_discord_multi_quote_message(selected_items: list[dict]) -> str:
    parts = [f"# {DISCORD_HEADER}", ""]

    for item in selected_items:
        author_id = str(item.get("author_id") or "").strip()
        mention = f"<@{author_id}>" if author_id.isdigit() else "@unknown"
        quote_text = truncate_text((item.get("text") or "").replace("\n", " ").strip(), QUOTE_MAX_TEXT_LEN)
        channel_id = str(item.get("channel_id") or "").strip()
        link = str(item.get("link") or "").strip()

        header = f"## Канал <#{channel_id}>" if channel_id.isdigit() else "## Канал"
        parts.append(header)
        parts.append(f"© {mention}: \"**{quote_text}**\"")
        if link:
            parts.append(link)
        parts.append("")
...
def split_message_for_discord(text: str, max_len: int) -> list[str]:
    if len(text) <= max_len:
        return [text]
    ...
    for block in text.split("\n\n"):
        ...
        if len(current) + len(piece) <= max_len:
            ...
            if len(block) <= max_len:
                ...
                current = ""
                for line in lines:
                    piece2 = line if not current else "\n" + line
                    if len(current) + len(piece2) <= max_len:
                        current += piece2
                    else:
                        if current:
                            chunks.append(current)
...
# =============================================================================
# Selected-only archive
# =============================================================================
def append_selected_only_record(path: str, record: dict):
    """
    File format: JSON array
    [
      {schema, generated_at, since, lookback_hours, dry_run, selected_per_channel},
      ...
    ]
    """
    arr = load_json(path, [])
    ...
    new_ids = {
        ...
        for item in (record.get("selected_per_channel") or [])
    ...
    if new_ids:
        for it in arr:
            old_ids = {
                str(item.get("message_id") or "").strip()
                for item in (it.get("selected_per_channel") or [])
                if str(item.get("message_id") or "").strip()
            }
            if old_ids == new_ids:
                return
...
# =============================================================================
def pick_best_quote_for_channel(channel_id: str, msgs: list[dict]) -> dict | None:
    all_quotes = []
    candidates = []
    ...
    return {
        ...
        "debug": {
            "kept_quotes_in_channel": len(all_quotes),
            "candidates_in_channel": len(candidates),
            "shortlist_in_channel": len(short_list),
        },
    }
...
def main():
    ...
    for idx, cid in enumerate(channel_ids, start=1):
        ...
        selected = pick_best_quote_for_channel(cid, msgs)
        ...
        run_channels.append(channel_debug)

        if selected:
            selected_per_channel.append(selected)
            logger.success(
                f"Selected quote for channel={cid} | "
                f"message_id={selected['message_id']} | author={selected.get('author_name', 'unknown')}"
            )
        else:
            logger.warning(f"No valid quote selected for channel={cid}")

    if not selected_per_channel:
        logger.warning("No quotes selected for any channel. Writing run output and exiting.")
        run_out = {
            "schema": 2,
            ...
            "debug": {
                ...
                "channels_with_selected_quote": 0,
            },
        }
    ...
    run_out = {
        ...
        "lookback_hours": LOOKBACK_HOURS,
        "dry_run": DRY_RUN,
        "selected_per_channel": selected_per_channel,
        "channels": run_channels,
        "debug": {
            "total_messages_scanned": total_msgs_scanned,
            "channels_total": len(channel_ids),
            "channels_with_selected_quote": len(selected_per_channel),
    ...
    if DRY_RUN:
        ...
        content = format_discord_multi_quote_message(selected_per_channel)
        chunks = split_message_for_discord(content, DISCORD_MAX_MESSAGE_LEN)
        post_results = []

        for i, chunk in enumerate(chunks, start=1):
            post_res = post_quote_to_discord(chunk)
            post_results.append(post_res)
            logger.success(f"Posted Discord message chunk {i}/{len(chunks)}")

        state["last_post_result"] = post_results

    state["last_run_utc"] = now.isoformat()
    state["last_since_utc"] = since_dt.isoformat()
    state["dry_run"] = DRY_RUN
    state["lang"] = QUOTE_LANG
    state["selected_channels_count"] = len(selected_per_channel)
